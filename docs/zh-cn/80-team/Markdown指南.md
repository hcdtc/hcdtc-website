...
MarkDown支持表格。
    ...
    第二行分割表头和内容。
    中划线（-）需要至少3个，否则有些编辑器无法识别
    文字默认居左
    中划线（-）两边加冒号（:）表示文字居中
    中划线（-）右边加冒号（:）表示文字居右
    注：原生的语法两边都要用 | 包起来，此处省略，而且有些编辑器不支持冒号的Align形式。
...
*   一个惊叹号 `!`
*   接着一个方括号，里面放上图片的替代文字，就是显示在图片下面的文字，相当于对图片内容的解释。
*   接着一个普通括号，里面放上图片的网址，最后还可以用引号包住并加上
    选择性的 'title' 文字。图片Title是图片的标题，当鼠标移到图片上时显示的内容。title可加可不加。
...
***本文来自***：
http://wow.kuapp.com/markdown/ ---> https://github.com/riku/Markdown-Syntax-CN/
...
* [Markdown 介绍](https://github.com/mzlogin/markdown-intro)